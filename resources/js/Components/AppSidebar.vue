...
export default {
  ...
  methods: {
    handleMenuClick(item) {
      ...
      this.filteredMails = shuffled.slice(0, Math.max(5, Math.floor(Math.random() * 10) + 1));
...
<template>
  <Sidebar class="overflow-hidden [&>[data-sidebar=sidebar]]:flex-row" v-bind="$props">
    ...
    <Sidebar collapsible="none" class="!w-[calc(var(--sidebar-width-icon)_+_1px)] border-r">
          ...
          <SidebarGroupContent class="px-1.5 md:px-0">
            <SidebarMenu>
              <SidebarMenuItem v-for="item in navMain" :key="item.title">
                <SidebarMenuButton :tooltip="renderTooltip(item.title)"
                  :is-active="activeItem && activeItem.title === item.title" class="px-2.5 md:px-2" @click="handleMenuClick(item)">
      ...
      <SidebarFooter>
        <NavUser class="block md:hidden" :user="user" />
      </SidebarFooter>
    </Sidebar>
    ...
    <Sidebar collapsible="none" class="hidden flex-1 md:flex">
      ...
      <SidebarContent>
        <SidebarGroup class="px-0">
          <SidebarGroupContent>
            <a v-for="mail in filteredMails" :key="mail.email" href="#"
